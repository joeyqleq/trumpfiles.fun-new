...
export default function CatalogClient({
    ...
    const filteredAndSortedEntries = useMemo(() => {
        ...
        filtered.sort((a, b) => {
            ...
            switch (sortBy) {
                case "entry_asc": return (a.entry_number || 0) - (b.entry_number || 0);
                case "entry_desc": return (b.entry_number || 0) - (a.entry_number || 0);
                case "score_desc": return parseFloat(b.fucked_up_score) - parseFloat(a.fucked_up_score);
                case "score_asc": return parseFloat(a.fucked_up_score) - parseFloat(b.fucked_up_score);
                case "danger_desc": return (b.danger || 0) - (a.danger || 0);
                case "lawlessness_desc": return (b.lawlessness || 0) - (a.lawlessness || 0);
                case "insanity_desc": return (b.insanity || 0) - (a.insanity || 0);
                case "absurdity_desc": return (b.absurdity || 0) - (a.absurdity || 0);
                case "date_desc": return getDate(b.date_start || undefined) - getDate(a.date_start || undefined);
                case "date_asc": return getDate(a.date_start || undefined) - getDate(b.date_start || undefined);
    ...
    const resetFilters = () => {
        setSearchTerm(""); setSelectedCategory("all"); setPhaseFilter("all"); setSortBy("entry_desc");
        setMinDanger([0]); setLocalMinDanger([0]); setMinAbsurdity([0]); setLocalMinAbsurdity([0]);
        setMinLawlessness([0]); setLocalMinLawlessness([0]); setMinTotalScore([0]); setLocalMinTotalScore([0]);
        setCurrentPage(1);
    };
                                ...
                                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                                    <div className="pt-6 border-t border-white/10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                                        <div className="space-y-4">
                                            <div className="flex justify-between items-center"><Label className="text-white flex items-center gap-2"><Flame className="w-4 h-4 text-red-500" />Min Danger</Label><span className="text-sm font-mono text-orange-400">{localMinDanger[0]}/10</span></div>
                                            <Slider value={localMinDanger} onValueChange={setLocalMinDanger} onValueCommit={setMinDanger} min={0} max={10} step={1} />
                                        </div>
                                        <div className="space-y-4">
                                            <div className="flex justify-between items-center"><Label className="text-white flex items-center gap-2"><Brain className="w-4 h-4 text-yellow-500" />Min Absurdity</Label><span className="text-sm font-mono text-orange-400">{localMinAbsurdity[0]}/10</span></div>
                                            <Slider value={localMinAbsurdity} onValueChange={setLocalMinAbsurdity} onValueCommit={setMinAbsurdity} min={0} max={10} step={1} />
                                        </div>
                                        <div className="space-y-4">
                                            <div className="flex justify-between items-center"><Label className="text-white flex items-center gap-2"><Gavel className="w-4 h-4 text-orange-500" />Min Lawlessness</Label><span className="text-sm font-mono text-orange-400">{localMinLawlessness[0]}/10</span></div>
                                            <Slider value={localMinLawlessness} onValueChange={setLocalMinLawlessness} onValueCommit={setMinLawlessness} min={0} max={10} step={1} />
                                        </div>
                                        <div className="space-y-4">
                                            <div className="flex justify-between items-center"><Label className="text-white flex items-center gap-2"><Zap className="w-4 h-4 text-purple-500" />Min Total Score</Label><span className="text-sm font-mono text-orange-400">{localMinTotalScore[0]}/100</span></div>
