...
const rows = await sql`
  ...
  FROM trump_entries
  WHERE entry_number BETWEEN 5539 AND 6000
    AND (sources IS NULL OR sources::text = 'null')
  ORDER BY entry_number
  LIMIT 60
`;

console.log(`Found ${rows.length} entries missing sources (5539–6000)`);
...
for (const row of rows) {
  ...
  try {
    ...
    for (const r of results) {
      if (isAllowedDomain(r.url)) { match = r; break; }
    }
    ...
    console.error(`! ${row.entry_number}: error — ${err.message}`);
    skipped++;
  }
}
